...
function inititalize_from_upstream() {
	if [[ -n "$UPSTREAM_GIT_URL" ]] && [[ -n "$UPSTREAM_GIT_BRANCH" ]]; then
		if [[ -n "$UPSTREAM_SOURCE_PACKAGE" ]]; then
			die "$PACKAGE/config.sh must contain either" \
				"UPSTREAM_SOURCE_PACKAGE or" \
				"UPSTREAM_GIT_URL/BRANCH, not both."
		fi
...
shift $((OPTIND - 1))
[[ $# -lt 1 ]] && usage "package argument missing"
...
#
# Package's config.sh file defines default values for:
#   GIT_URL, GIT_BRANCH, VERSION, REVISION.
# Those defaults can be overriden in get_settings_from_env().
#
logmust get_settings_from_env
